...
Tunnel.prototype._init = function(cb) {
    ...
    // where to quest
    var uri = base_uri + ((assigned_domain) ? assigned_domain : '');
    
    if (opt.token) {
        uri = uri + `?token=${opt.token}`;
    }
    ...
    (function get_url() {
        ...
        .catch(function(err){
            ...
            if (err.response.status === 511) { // Authentication error. Token is required.
                console.log(`Authentication error: token is required.`);
                process.exit(1);
            } else {
